...
class VideoRecombinator(NamedTuple):
    ...
    def main(self):
        ...
        vd = VideoDescriptor.from_serialized(self.conf)
        audio_path = vd.audio_path
        frame_indices = json.load(open(self.frame_config, "r"))

        # 2 - Regenerate video elements with the new frame config
        for ve, frame_index in zip(vd.all_video_elements, frame_indices):
        ...
        concat_file_path = f"{self.output_dir}/temp.txt"
